 ...
 * 입력창의 값을 읽고 유효성을 검사한 후 API를 호출합니다.
 */
async function handleSearch() {
  ...
  // 2-3. 입력값이 비어있는지 확인하기
  // !keyword는 keyword가 빈 문자열("")일 때 true가 됩니다.
  if (!keyword) {
  ...
  // 2-4. 상태 메시지 업데이트
  statusMessage.textContent = `"${keyword}" 검색 중...`;
  
  try {
    // 2-5. fetchRepos 함수로 GitHub API 호출
    // await는 API 응답을 받을 때까지 기다립니다.
    const data = await fetchRepos(keyword);
    
    // 2-6. renderRepos 함수에 items 배열 전달
    // data.items는 검색된 저장소 목록입니다.
    renderRepos(data.items);
    
    // 2-7. 검색 완료 메시지 표시
    statusMessage.textContent = `"${keyword}" 검색 완료! ${data.items.length}개의 저장소를 찾았습니다.`;
    
  } catch (error) {
    // 2-8. 에러 발생 시 처리
    console.error('검색 중 오류 발생:', error);
    statusMessage.textContent = '검색 중 오류가 발생했습니다. 다시 시도해 주세요.';
  }
}
